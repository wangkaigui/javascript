...
class WxManageController extends AdminbaseController{
    ...
    public function _initialize(){
        ...
        $datas = json_decode($this->get_php_file("./data/access_token.php"));
        if ($datas->expire_time < time()) {
            ...
            if ($arr['access_token']) {
                ...
                $this->set_php_file("./data/access_token.php", json_encode($datas));
                }
        } else {
    ...
    public function ajaxSend($object){
        if($object){
    ...
    public function echoMsg(){
        $openid = I('get.openid');
        $timed = date('Y-m-d H:i:s', strtotime('-3 day'));
        $time = date('Y-m-d H:i:s',time());
        $data = M('')->query("select t.*,w.nickname,w.headimgurl from jbr_test as t left join jbr_weixin_userlist as w on t.openid=w.openid where t.created between '$timed' AND '$time' AND t.openid='$openid' group by t.id order by t.created asc");
    ...
    public function ajaxMsg(){
        ...
        $timed = date('Y-m-d H:i:s', strtotime('-2 second'));
        $time = date('Y-m-d H:i:s',time());
        $data = M('')->query("select t.*,w.nickname,w.headimgurl from jbr_test as t left join jbr_weixin_userlist as w on t.openid=w.openid where t.created between '$timed' AND '$time' AND t.openid='$openid' group by t.id order by t.created asc");
    ...
    public function sendMsg(){
        ...
        echo request_post($url, json_encode($data,JSON_UNESCAPED_UNICODE));
        
        }
        
    }
    
    public function ajaxAddMsg($keyword,$openid){
        if($keyword){
             $data = array(
                'words'=>$keyword,
                'created'=>date("Y-m-d H:i:s",time()),
    ...
    private function receiveText($object)
    {
        ...
        $list = M('weixin_huifu')->where('leixin=1')->select();
        foreach ($list as $k=>$v){
...
$item_str    </Articles>
    ...
    public function addMenu(){
        ...
        $data .= ']}';
        $menuback = json_decode(request_post($post,$data),true);
    ...
    public function delmenu(){
        ...
        if(M('weixin_menu')->where("id=$id")->setField('deleted',1)){
            $this->success('删除成功');
        }else{
            $this->error('删除失败');
        }
