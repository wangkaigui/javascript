...
class IndexController extends Base {
	...
	public function send(){
		...
		// 推送的url地址，使用自己的服务器地址
		$push_api_url = "192.168.3.24:2121";
		$post_data = array(
		   "type" => "publish",
		   "content" => $content,
		   "to" => $to_uid, 
	...
	public function getChatContent(){
		if(IS_POST){
			$uid = I('post.uid');
			$odays =  date('Y-m-d H:i:s', strtotime("+30 days"));
			$sql = "select * from jbr_web_chat where fid='$uid' and time < '$odays' ";
			$data = M()->query($sql);
	...
	public function tests(){
		//echo 'http://img.ksbbs.com/asset/Mon_1605/0ec8cc80112a2d6.mp4';
		$this->assign('video',"['http://img.ksbbs.com/asset/Mon_1605/0ec8cc80112a2d6.mp4->video/mp4']");
		$this->display();
	}
}
