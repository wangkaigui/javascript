...
<script>
		...
		//获取消息记录
		var msg='';
		$.post("{:U('Index/getChatContent')}",{'uid':uid},function(result){
			//alert(result);
...
$(document).ready(function () {
    ...
    socket.on('update_online_count', function(data){
		/*$.post("{:U('Chat/ajaxSetUser')}",{'uid':uid},function(result){
...
function send(){
	...
	$.post("{:U('Index/send')}",{'uid':toUser,'fuid':uid,'content':$("input[name='content']").val()},function(result){
